...
@IBDesignable public class TrimmerView: AVAssetTimeSelector {
    ...
    private var normalizedPosition = CGFloat(0.0)
...
//        layer.masksToBounds = true
    ...
    override func constrainAssetPreview() {
        ...
        //assetPreview.backgroundColor = .red
    }
    ...
    private func setupTrimmerView() {
        ...
        //trimView.layer.backgroundColor = UIColor(red: 0/255, green: 0/255, blue: 0/255, alpha: 0.4).cgColor
        addSubview(trimView)
    ...
    private func setupMaskView() {
        ...
        maskImageView.image = UIImage(named: "BarImage")
        maskImageView.translatesAutoresizingMaskIntoConstraints = false
        trimView.addSubview(maskImageView)
        
        maskImageView.heightAnchor.constraint(equalToConstant: 30).isActive = true
        maskImageView.widthAnchor.constraint(equalToConstant: 2).isActive = true
        maskImageView.leftAnchor.constraint(equalTo: self.trimView.leftAnchor, constant: 10).isActive = true
       // maskImageView.rightAnchor.constraint(equalTo: self.leftHandleView.rightAnchor, constant: 0).isActive = true
        //maskImageView.bottomAnchor.constraint(equalTo: self.trimView.bottomAnchor, constant: 0).isActive = true
        maskImageView.topAnchor.constraint(equalTo: self.trimView.topAnchor, constant: 15).isActive = true
        ...
        let markRightImageView = UIImageView()
        
        
        markRightImageView.image = UIImage(named: "BarImage")
        markRightImageView.translatesAutoresizingMaskIntoConstraints = false
        trimView.addSubview(markRightImageView)
        
        markRightImageView.heightAnchor.constraint(equalToConstant: 30).isActive = true
        markRightImageView.widthAnchor.constraint(equalToConstant: 2).isActive = true
        markRightImageView.rightAnchor.constraint(equalTo: self.trimView.rightAnchor, constant: -10).isActive = true
       // maskImageView.rightAnchor.constraint(equalTo: self.leftHandleView.rightAnchor, constant: 0).isActive = true
        //maskImageView.bottomAnchor.constraint(equalTo: self.trimView.bottomAnchor, constant: 0).isActive = true
        markRightImageView.topAnchor.constraint(equalTo: self.trimView.topAnchor, constant: 15).isActive = true
    ...
    private func setupPositionBar() {
        ...
        //positionBar.frame = CGRect(x: 0, y: -5, width: 3, height: frame.height + 10)
        ...
        positionBar.heightAnchor.constraint(equalTo: rightHandleView.heightAnchor, constant: 0).isActive = true
    ...
    public func seek(to time: CMTime) {
        if let newPosition = getPosition(from: time) {
            ...
            let maxPosition = rightHandleView.frame.origin.x - (leftHandleView.frame.origin.x + handleWidth)
             ...
             normalizedPosition = min(max(0, offsetPosition), maxPosition)
            
            //timer = Timer.scheduledTimer(timeInterval: 0.01, target: self, selector: #selector(positionBarMove), userInfo: nil, repeats: true)
    ...
    private func updateSelectedTime(stoppedMoving: Bool) {
        ...
        if stoppedMoving {
           // print("Check: Player Time: ", playerTime)
